 ...
 *
 *
 * @copyright  Copyright (c) 2007-2013 ShopNC Inc. (http://blog.whobt.com)
 * @license    http://blog.whobt.com
...
class taobao_importControl extends BaseSellerControl {
	...
	public function indexOp(){
		...
		if(!$_POST){
			...
			$gc_sub_list	=	$gc->getGoodsClass(array('gc_parent_id'=>intval($_POST['gc_id'])));
			...
			if(is_array($_POST['sgcate_id']) and count($_POST['sgcate_id']) > 0){
				foreach ($_POST['sgcate_id'] as $sgcate_id) {
					if(!in_array($sgcate_id,$sgcate_ids)){
						$stc_row	= $stc->getStoreGoodsClassInfo($sgcate_id);
			...
			/* 兼容淘宝助理5 start */
			$csv_array = explode("\to2o_bind_service", $csv_string, 2);
			//print_r($csv_array);
		   ...
		   if (strtoupper(CHARSET) == 'GBK'){
			...
			if(is_array($records) and count($records) > 0){
				foreach($records as $k=>$record){
					...
					$goods_class = Model('goods_class')->getGoodsClassLineForTag(intval($_POST['gc_id']));
					

					$param	= array();
					$param['goods_name']			= $record['goods_name'];
					$param['gc_id']					= intval($_POST['gc_id']);
					$param['gc_id_1']				= intval($goods_class['gc_id_1']);
					$param['gc_id_2']				= intval($goods_class['gc_id_2']);
					$param['gc_id_3']				= intval($goods_class['gc_id_3']);
					$param['gc_name']				= htmlspecialchars($gc_row['gc_tag_name']);
					...
					if($goods_id){
						 ...
						 */
					  	...
					  	if(!empty($pic_array['goods_image']) && is_array($pic_array['goods_image'])){
							...
							$rs = $model_store_goods->addGoodsAll($insert_array);
		...
		if(trim($_GET['goods_id_str'])==''){
			...
			if(is_array($gid_arr) && !empty($gid_arr)){
				...
				foreach($gid_arr as $v1){
					...
					$goods_image		= '';			// 商品默认图
					$i=1;
					foreach((array)$upload_list as $k2=>$v2){
					...
					$goods_model->addGoodsImagesAll($insert_array);
