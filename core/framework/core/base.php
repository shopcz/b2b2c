...
final class Base{
	...
	private static function parse_conf(&$setting_config){
		...
		$setting['shopnc_version'] = '<span class="vol"><font class="b">网店</font><font class="o">交流</font></span>';
	...
	private static function start_session(){
		...
		//开启以下配置支持session信息存信memcache
		 ...
		 // @ini_set("session.save_path", "tcp://".C('memcache.1.host').':'.C('memcache.1.port'));

		 //开启以下配置支持session信息存信radis
		 //  @ini_set("session.save_handler", "redis");
		 // @ini_set("session.save_path", "tcp://".C('redis.master.host').':'.C('redis.master.port'));
