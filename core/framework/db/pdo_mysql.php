...
class Db{
	...
	private function __construct(){
		if(!extension_loaded("pdo_mysql")){
			throw_exception("Db Error:pdo_mysql is no install");
		}
	}
	...
	private static function connect($host = 'slave'){
		...
		try{
		self::$link[$host] = @new pdo("mysql:host={$conf['dbhost']};dbname={$conf['dbname']};dbport={$conf['dbport']}",$conf['dbuser'],$conf['dbpwd']);
		...
		if (!self::$link[$host]->query($query_string)){
			throw_exception("Db Error: PDO query error code is".self::$link[$host]->errorCode());
		}
	}
	 ...
	 * @param strIng $sql
	 * @return mixed 
	 */public static function query($sql, $host = 'master',$type='exec'){
			self::connect($host);
			...
			if($query === false){
				$error = 'Db Error: PDO query error code is '.self::$link[$host]->errorCode();
				if(C('debug')){
	...
	public static function getAll($sql, $host = 'slave'){
		self::connect($host);
		$result = self::query($sql, $host,"query");
	...
	public static function select($param, $obj_page='', $host = 'slave'){
		...
		if ($obj_page instanceof Page ){
			$count_query = self::query($count_sql,$host,"query");
			$count_fetch = $count_query->fetch(PDO::FETCH_ASSOC);
	...
	public static function update($table_name, $update_array = array(), $where = '', $host = 'master'){
		...
		$result = self::query($sql, $host,$type='query');
		if($result->errorCode() ==='00000'){return true;	}else{return false;} 	
	...
	public static function getLastId($host = 'master'){
		...
		if (!$id){
		    $result = self::query('SELECT last_insert_id() as id',$host,'query');
		    if ($result === false) $result = array();
	...
	public static function getServerInfo($host = 'slave'){
		self::connect($host);
		$result = self::$link[$host]->getAttribute(PDO::ATTR_SERVER_VERSION);
		return $result;
	}
    ...
    public static function commit($host = 'master'){
    	if (!self::$iftransacte){
    		...
    		if (!$result) throw_exception("Db Error: ".var_dump(self::$link[$host]->errorInfo()));
    	}
    }
